...
if(isset($_POST['heatstarttime'])){
    $heatstarttime = $_POST['heatstarttime'];
}

if(isset($_POST['heatendtime'])){
...
echo "diff"; echo $totalheattime;
echo "<br>";

$sTime_only=date('H:i', strtotime($heatstarttime));
$eTime_only=date('H:i', strtotime($heatendtime));
echo $sTime_only;
echo $eTime_only;
...
$sql_1 = "select sum(mr_production) from breakdown where  date='" . $per_date . "' and heat_number= '" . $heatnumber . "' and location_code in(7,8,9,10,12,13,16,17)";
...
$sql_ph = "insert into `per_heat_production`(`perheatdate`,`roughing`,`heat-number`,`heat-start-time`,`heat-end-time`,`total-heat-time`,`total_bd_time`,"
        ...
        . " `totalcuttingprod`,`totalrolledpcs`,`shift`)"
        . " values"
        ...
        . "'"  .$T32cut."' ,'". $totalrfmrprod . "','" . $totalcuttingprod . "','".$totalrolledpcs."','".$shift."')";
